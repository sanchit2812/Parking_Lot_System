...
int main(){
    ...
    if (myfile.is_open()){
        ParkingLot obj; // Creation of an object of class Parking_Lot
        ...
        while ( getline (myfile,s) ){ // getting input till there is a line to take input
            ...
            // Case: when Car is going to Leave
                ...
                pair<string, int> car_info = obj.leave(toInt(v[1]));
                if(car_info.first == "-2") minefile<<"Slot is not available in this parking area"<<endl;
                    ...
                    minefile <<"Slot Number : "<<toInt(v[1])<<" vacated, the car with vehicle registration number :"<<car_info.first<<" left the space."<<endl;
                }
            }
            ...
            // Case: when Vehicle registration number required for driver of given age
                ...
                vector<string> vec = obj.getRegNoFromAge(toInt(v[1]));
                if(vec.size()==0){
            ...
            // Case: when Slot numbers is required for driver of given age
                ...
                vector<int> vec = obj.getSlotNoFromAge(toInt(v[1]));
                if(vec.size()==0){
                    minefile <<"NO Car is present"<<endl;
                }
                else{
